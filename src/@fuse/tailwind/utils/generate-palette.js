...
const generatePalette = (config) => {
    ...
    ;
const generatePaletteRGB = (config) => {
    // Prepare an empty palette
    const palette = {
        50: null,
        100: null,
    ...
    // Build and return the final palette
    return {
        50: scale(0.05).rgb().join(","),
        100: scale(0.1).rgb().join(","),
        200: scale(0.2).rgb().join(","),
        300: scale(0.3).rgb().join(","),
        400: scale(0.4).rgb().join(","),
        500: scale(0.5).rgb().join(","),
        600: scale(0.6).rgb().join(","),
        700: scale(0.7).rgb().join(","),
        800: scale(0.8).rgb().join(","),
        900: scale(0.9).rgb().join(",")
    };
};

module.exports = { generatePalette, generatePaletteRGB };
