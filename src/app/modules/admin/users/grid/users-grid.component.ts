...
import { User, UserTag, UserPagination } from '../users.types';
import { InventoryVendor } from '../../tenants/tenants.types';
...
export class UsersGridComponent implements OnInit, AfterViewInit, OnDestroy {
     ...
     * @param event
     */
    updateTagTitle(tag: UserTag, event): void {
     ...
     * @param tag
     */
    removeTagFromUser(tag: UserTag): void {
        // Remove the tag
     ...
     * @param change
     */
    toggleUserTag(tag: UserTag, change: MatCheckboxChange): void {
        if (change.checked) {
            this.addTagToUser(tag);
        } else {
            this.removeTagFromUser(tag);
