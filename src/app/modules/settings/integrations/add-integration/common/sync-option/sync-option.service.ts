...
export class SyncOptionService {
    ...
    createIntegration(integrationValue: MappedIntegration): Observable<MappedIntegration> {
        ...
        return this._httpClient.post(api, integrationValue).pipe(
            map((x: any) => x.result),
            tap(_result => this._snackbarService.showSuccess('Successfully saved integration configuration')),
            catchError(({ error }) => {
                this._snackbarService.showError(error?.errors?.name || 'Failed to save integration configuration');
                return of();
            })
        )
    }
