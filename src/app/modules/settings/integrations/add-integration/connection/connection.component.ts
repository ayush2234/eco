...
export class AddIntegarationConnectionComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    console.log('Instance', this.instance.integration);
  ...
  getApiSyncOptions() {
    return this.instance.integration.sync_options.map(option => {
      return {
        ...
        is_activated:
          option.is_activated !== undefined ? option.is_activated : false,
        sub_sync_options: [],
      };
    });
  }
  ...
  eliminateUnchanged(objectIntegrationValue: IntegrationValue) {
    this.instance.integration.connection.fields.forEach(field => {
      if (
        objectIntegrationValue.connection[field.code] ===
        this.mappedIntegration.connection[field.code]
      ) {
  ...
  mapFormToInstance() {
    ...
    this.mappedIntegration = {
      ...this.mappedIntegration,
      connection: this.verificationData
        ? { ...integrationClone.connection, ...this.verificationData }
        : { ...integrationClone.connection },
      integration_id: this.isAddIntegration
        ? this.instance.integration_id
        : this.instance.integration.integration_id,
      sync_options: this.mappedIntegration.sync_options.map(x => ({
        ...x,
        is_active: this.instance.integration.sync_options.find(
          s => s.code === x.code
        )?.is_active,
        is_activated: this.instance.integration.sync_options.find(
          s => s.code === x.code
        )?.is_activated,
      })),
    };
  ...
  messageListener(event: MessageEvent<any>) {
    if (event?.data?.error)
      this._snackbarService.showError(
        'Please check domain Url, some error occured !!'
      );
    else {
      this.verificationData = { ...event?.data };
      if (this.verificationData && this.verificationData?.access_token)
        this._snackbarService.showSuccess('Verification Successful !!');
  ...
  addIntegration() {
    ...
    this._syncOptionService
      .createIntegration(this.mappedIntegration)
      ...
      .subscribe(integration => {
        ...
        if (integration) {
          ...
          this._integrationsService
            ...
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe((response: IntegrationSettings) => {
